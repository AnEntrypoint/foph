...
const TEST_HOME = path.join(os.tmpdir(), 'foph-test-' + Date.now())
process.env.FOPH_HOME = TEST_HOME; process.env.FOPH_PROFILES_ROOT = path.join(TEST_HOME, 'profiles')
...
await T('home+config+skin', async () => {
    const { getFophHome } = await import('./src/home.js'); assert.equal(getFophHome(), TEST_HOME)
...
await T('tools+toolsets', async () => {
    ...
    assert.match(JSON.parse(await registry.dispatch('bash', { command: 'echo hi-foph', timeout_ms: 5000 })).stdout, /hi-foph/)
    const { getEnabledToolSchemas, _FOPH_CORE_TOOLS } = await import('./src/toolsets.js')
    assert.ok((await getEnabledToolSchemas(['core'])).length >= _FOPH_CORE_TOOLS.length)
})
...
await T('acp-full', async () => {
    ...
    assert.equal(lines[0].result.name, 'foph'); assert.ok(lines[0].result.events.includes('tool.start'))
...
await T('profiles+observability+auth+env+context+cron+batch+slash+skills', async () => {
    ...
    const sv = await import('./src/gateway/service.js'), oe = await import('./src/agent/oauth_endpoints.js'); assert.ok(sv.getRuntimeSnapshot().serviceName.startsWith('foph-gateway') && /Environment=K=V/.test(sv.renderSystemdUnit({ execStart: 'x', envVars: { K: 'V' } })) && oe.resolveKimiBaseUrl().includes('moonshot') && oe.resolveZaiBaseUrl({ endpoint: 'https://api.z.ai' }).includes('z.ai') && oe.isCodexAccessTokenExpiring('bad.jwt'))
...
await T('mcp+swe+distributions+account+credpool', async () => {
    ...
    const lines = out.join('').trim().split('\n').map(l => JSON.parse(l))
    assert.equal(lines[0].result.serverInfo.name, 'foph-mcp'); assert.ok(lines[1].result.tools.length >= 50); srv.stop()
