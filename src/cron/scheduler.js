...
export function createJob({ cron, prompt, model = null }) {
    parseCron(cron)
    const d = init()
...
export function listJobs() {
    return init().prepare(`SELECT * FROM cron_jobs ORDER BY id DESC`).all()
}

export function cancelJob(id) {
    init().prepare(`UPDATE cron_jobs SET enabled = 0 WHERE id = ?`).run(id)
}

export function deleteJob(id) {
    init().prepare(`DELETE FROM cron_jobs WHERE id = ?`).run(id)
}
